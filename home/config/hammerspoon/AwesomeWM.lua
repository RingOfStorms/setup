...
local function updateFrame(state)
...
local function grabWindow(state, event)
...
local function releaseWindow(state)
  if state.selectedWindow then
...
local setup = function(options)
  ...
  local function grabWindowConnected(event)
    return grabWindow(state, event)
  end

  local function releaseWindowConnected()
    return releaseWindow(state)
  end
  U.persist(
    hs.eventtap.new({ hs.eventtap.event.types.leftMouseDown }, grabWindowConnected):start(),
    hs.eventtap.new({ hs.eventtap.event.types.rightMouseDown }, grabWindowConnected):start(),
    hs.eventtap.new({ hs.eventtap.event.types.leftMouseUp }, releaseWindowConnected):start(),
    hs.eventtap.new({ hs.eventtap.event.types.rightMouseUp }, releaseWindowConnected):start()
  )
